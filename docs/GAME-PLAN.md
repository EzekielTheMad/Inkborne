...
2. **Tests gate merges.** Run `npm run check` before any PR. GitHub CI, GitGuardian, and the Vercel preview must also be green before merge. The suite is **1,666 tests across 167 files as of 2026-07-21**.
...
**Not built yet:** broader importer missing-info editing; the remaining M4 authoring/content types; optional public publishing; monsters/library NPCs (M5); campaign live presence (later); and public-beta polish.
...
2. **M4 — Homebrew + importer:** ◑ **Spell and feat authoring/sharing plus the private importer workflow through calculation preview are shipped.** Campaign isolation, exact-version pins, DM revocation, ASI feat selection, fail-closed parsing/mapping, guided repair, conflict resolution, and the current-revision calculation gate are hosted-database and protected-preview verified. Next: broaden schema-known importer repairs; then add authoring/content types and optional public publishing.
