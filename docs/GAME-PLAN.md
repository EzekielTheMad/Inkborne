...
**Not built yet:** homebrew authoring/import (M4), monsters/NPCs (M5), campaign public publishing/live presence (later M5.5), and public-beta polish. The campaign foundation, membership lifecycle, character copy/assignment, wiki tree/editor, bidirectional character narrative links/backlinks, and revision-safe collaboration are implemented on the campaign branch; migrations `00041`–`00054` are deployed to production. Authenticated DM/player browser UAT is green, but the application branch still needs to merge and deploy before the campaign UI is live on `inkborne.app`.
...
4. **M5.5 — Campaigns + narrative depth** *(in progress 2026-07-19 — campaign foundation implemented before M5)*: campaign CRUD/membership, DM/player roles, character copy/assignment, wiki tree/editor, bidirectional character↔campaign narrative links/backlinks, visibility controls, and revision conflicts are implemented; production migrations, rollback-only database smoke UAT, and authenticated DM/player browser UAT are complete. Remaining: merge/deploy the application branch, timeline/relationship expansion, and later publishing/presence decisions.
