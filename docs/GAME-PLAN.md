...
**Not built yet:** the remaining M4 content types, broader importer missing-info editing, conflict resolution, preview-character validation, optional public publishing, monsters/library NPCs (M5), campaign live presence (later M5.5), and public-beta polish. The campaign foundation, membership lifecycle, character copy/assignment, wiki tree/editor, bidirectional character narrative links/backlinks, timelines/relationships, visibility controls, revision-safe collaboration, first spell-sharing loop, private MPMB parse/map/review foundation, and guided spell repair are implemented and hosted-database verified; application branches still need to merge and deploy before all of that UI is live on `inkborne.app`.
...
2. **M4 — Homebrew + importer:** ◑ **Spell sharing and the private importer foundation are in release review.** Spell authoring, immutable edits, campaign-scoped sharing, character-aware discovery, exact-version sheet pinning, fail-closed MPMB parsing/mapping, and owner-only import review/commit are implemented and hosted-UAT verified. Remaining: other content types, missing-info editing, conflict resolution, preview-character validation, and optional public publishing.
