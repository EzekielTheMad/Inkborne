...
**In release review:** the M4 multiplayer and importer verticals now provide `/library` spell authoring, immutable versions, campaign-scoped sharing, character-aware discovery, exact-version sheet pinning, fail-closed MPMB parsing, guided repair, explicit content-conflict resolution, and a mandatory server-calculated preview before commit. Hosted coverage proves campaign isolation, immutable pins, atomic import behavior, and the exact-revision preview gate.

**Not built yet:** the remaining M4 authoring/content types, broader importer missing-info editing, optional public publishing, monsters/library NPCs (M5), campaign live presence (later M5.5), and public-beta polish. The campaign foundation, membership lifecycle, character copy/assignment, wiki tree/editor, bidirectional character narrative links/backlinks, timelines/relationships, visibility controls, revision-safe collaboration, first spell-sharing loop, and the private MPMB workflow through calculation preview are implemented and hosted-database verified; application branches still need to merge and deploy before all of that UI is live on `inkborne.app`.
...
2. **M4 — Homebrew + importer:** ◑ **Spell sharing and the private importer workflow through calculation preview are in release review.** Spell authoring, immutable edits, campaign-scoped sharing, character-aware discovery, exact-version sheet pinning, fail-closed MPMB parsing/mapping, guided repair, conflict resolution, and a current-revision calculation gate are implemented and hosted-database verified. Remaining: other authoring/content types, broader missing-info editing, and optional public publishing.
...
- **2026-07-21** — M4 MPMB calculation preview. Added an owner-only server-rendered calculation harness that evaluates every selected feat independently at levels 1/5/11/17 and test-casts every selected spell across its supported scaling range. Raw staged rules remain server-only; malformed schemas, formulas, dice, or calculations fail closed. Confirmation stamps only the exact import revision through a service-role-only RPC, and the public commit RPC refuses unstamped or stale revisions. Live-sheet structured feat sources now match preview behavior. Hosted rollback smoke also exposed and repaired a pre-existing renamed selection-function regression without leaving fixtures. Verification: strict lint/typecheck, production build, **157 Vitest files / 1584 tests**, hosted migration dry-run/apply, exact-revision commit/selection/idempotency smoke, zero preview-related advisor findings, and zero fixture residue. Protected-preview browser UAT remains before merge.
