...
**In release review:** the first M4 authoring vertical now provides a private `/library` spell workflow, immutable version creation, owner-only discovery, and exact-version character-sheet pinning. Manual two-account browser UAT and automated live-backend Playwright coverage prove v1 pins remain unchanged after a v2 edit and that private content does not cross account boundaries.

**Not built yet:** the remaining M4 content types, sharing/importer/preview workflow, monsters/library NPCs (M5), campaign public publishing/live presence (later M5.5), and public-beta polish. The campaign foundation, membership lifecycle, character copy/assignment, wiki tree/editor, bidirectional character narrative links/backlinks, timelines/relationships, and revision-safe collaboration are implemented on the campaign branch; migrations `00041`–`00055` are deployed to production. Authenticated DM/player browser UAT is green, but the application branch still needs to merge and deploy before the campaign UI is live on `inkborne.app`.
...
2. **M4 — Homebrew + importer:** ◑ **First vertical in release review.** Private spell authoring, immutable edits, owner-only discovery, and exact-version sheet pinning are implemented and UAT-verified. Remaining: other content types, campaign/public sharing, MPMB JS import pipeline (reuse `scripts/transformers/`), and preview-character validation.
