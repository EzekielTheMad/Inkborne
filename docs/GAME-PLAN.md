...
**Not built yet:** effect-coupled/candidate-less importer repair; the remaining M4 authoring/content types and imported-content builder proof; optional public publishing; monsters/library NPCs (M5); campaign live presence (later); and public-beta polish.
...
2. **M4 — Homebrew + importer:** ◑ **Spell and feat authoring/sharing plus the private importer workflow through calculation preview and finite schema-known repair are shipped.** Campaign isolation, exact-version pins, DM revocation, ASI feat selection, fail-closed parsing/mapping, audited spell/feat repair, conflict resolution, and the current-revision calculation gate are hosted-database and protected-preview verified. Next: add authoring/content types and prove imported content through the character builder; optional public publishing stays separate.
