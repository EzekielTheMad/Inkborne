...
**Status:** implemented; database apply and authenticated UAT complete
**Date:** 2026-07-19
...
- Migrations `00041`–`00051` are applied to the hosted Supabase project.
- Static migration contracts, authorization/unit tests, strict lint, TypeScript, and the production build pass.
- Authenticated two-account browser UAT covers hidden/shared pages, visibility-safe backlinks, DM read-only character entry points, character assignment, and campaign joining. Rollback-only database UAT covers character copy, invite rotation, leave/remove cleanup, and stale page revisions.
