...
- Authenticated feedback capture records the current page path/query and browser context, with secure storage and an admin triage dashboard. Persistent desktop/mobile access is implemented in the active release candidate; the protected-preview submission check remains before closed alpha.
...
**Active M4 work:** atomic background replacement and exact pinned snapshots are hosted/runtime verified in the active release candidate. Finish protected-preview UAT, then build the entitlement-aware player compendium before continuing through the remaining character-construction content types. Optional public publishing remains deliberately separate from campaign sharing.
