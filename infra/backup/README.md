...
with `--exit-on-error`, prints key row counts, and removes all temporary
artifacts on exit.
...
The cross-platform contract suite includes shell syntax checks and injected
failures for dump, Storage, restic, locking, and alert behavior:

```bash
node --test infra/backup/tests/backup-contracts.node.mjs
```

CI also renders the Compose configuration and builds the backup image on Linux.
...
Confirm `PGURI` is the Dashboard's Session pooler connection string: tenant-qualified username `postgres.<project-ref>`, the displayed regional pooler host, and port 5432. If the database password rotated, URL-encode any reserved characters, update `.env`, and run `docker compose restart`.
