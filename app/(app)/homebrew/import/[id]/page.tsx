...
export default async function MpmbImportReviewPage({
  ...
  return (
    <div className="mx-auto w-full max-w-5xl space-y-7">
      <Link href="/homebrew" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
          ...
          {error
            ?? (repaired
              ...
              : resolved
                ...
                : `${committed} ${committed === 1 ? "definition" : "definitions"} added to your private Homebrew collection.`)}
        </div>
      )}
                ...
                <div className="flex flex-col items-stretch gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex w-full flex-col items-stretch gap-2 sm:w-auto sm:shrink-0 sm:items-end">
                    ...
                    {item.repairable && !completed && review.status === "review" && (
                      <Link
                        href={`/homebrew/import/${review.id}/items/${item.id}/edit`}
                    ...
                    {(item.hasLiveConflict || item.conflictResolution !== null)
                      && !completed && review.status === "review" && (
                      <Link
                        href={`/homebrew/import/${review.id}/items/${item.id}/conflict`}
                        className={buttonVariants({ variant: "outline", size: "sm" })}
                      >
          ...
          <>
            ...
            <div className="flex flex-col items-end gap-1.5">
              {selectedCount > 0 && unresolvedSelectedConflicts === 0 ? (
                <Link
                  href={`/homebrew/import/${review.id}/preview`}
