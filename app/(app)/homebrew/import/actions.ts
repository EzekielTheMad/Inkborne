...
export async function startMpmbImport(
  ...
  redirect(`/homebrew/import/${result.importId}`);
}
...
export async function toggleMpmbImportItem(
  ...
  if (result.status === "success") revalidatePath(`/homebrew/import/${importId}`);
  return result;
}
...
export async function repairMpmbImportSpell(
  ...
  revalidatePath(`/homebrew/import/${parsed.data.import_id}`);
  redirect(`/homebrew/import/${parsed.data.import_id}?repaired=1`);
}
...
export async function resolveMpmbImportConflict(
  ...
  const reviewPath = `/homebrew/import/${parsed.data.import_id}`;
...
export async function finishMpmbImport(formData: FormData): Promise<void> {
  ...
  if (result.status !== "success") {
    redirect(
      `/homebrew/import/${encodeURIComponent(importId)}?error=${encodeURIComponent(result.message)}`,
    );
  }
  revalidatePath("/homebrew");
  revalidatePath("/library");
  revalidatePath(`/homebrew/import/${importId}`);
  redirect(
    `/homebrew/import/${importId}?committed=${result.importedCount ?? 0}`,
  );
}
...
export async function confirmMpmbImportPreview(
  ...
  if (!parsed.success) {
    redirect("/homebrew/import?error=The%20preview%20confirmation%20is%20invalid.");
  }
  ...
  const previewPath = `/homebrew/import/${parsed.data.import_id}/preview`;
  if (result.status !== "success") {
