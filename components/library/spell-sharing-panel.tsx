...
interface SpellSharingPanelProps {
  contentId: string;
  version: number;
...
const initialState: HomebrewSpellShareActionState = {
  status: "idle",
  message: "",
};
...
function versionEffect(
  ...
  if (!shared && scope === "personal") {
    return `Sharing creates version ${version + 1} because this spell becomes Shared.`;
  }
  if (shared && sharedCampaignCount === 1) {
    return `Removing final access creates version ${version + 1} because this spell becomes Private.`;
  }
  return shared
...
export function SpellSharingPanel({
  contentId,
  version,
  scope,
  sharedCampaignCount,
  campaigns,
}: SpellSharingPanelProps) {
  ...
  return (
    <section className="j-card-paper space-y-5 p-5 sm:p-7" aria-labelledby="campaign-access-heading">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2">
            <Users className="size-4 text-accent" aria-hidden="true" />
            <h2 id="campaign-access-heading" className="j-folio">Campaign access</h2>
          </div>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
            Share this spell with campaigns you belong to. Campaign members can discover the current
            version, but only you can edit it.
          </p>
        </div>
      ...
      <div className="rounded-lg border border-border/70 bg-muted/30 px-4 py-3 text-xs leading-relaxed text-muted-foreground">
        <div className="flex items-start gap-2">
          ...
          <p>
            The first share and final unshare create an immutable version when the spell changes
            between Private and Shared. Existing character pins remain unchanged and usable.
          </p>
        </div>
      </div>
      ...
      {campaigns.length === 0 ? (
        <p className="rounded-lg border border-dashed border-border px-4 py-6 text-center text-sm text-muted-foreground">
          Join or create a D&amp;D 5e (2014) campaign to share this spell.
        </p>
      ) : (
